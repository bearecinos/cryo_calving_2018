# This will run glen a x factors experiment only for MT
...
for f in factors:
    suf = '_{:03d}_glenaxfactors_k1_cfgFS'.format(int(f * 10))

    if RUN_INVERSION:
        # Inversion tasks
        execute_entity_task(tasks.prepare_for_inversion, gdirs)
        execute_entity_task(tasks.volume_inversion, gdirs, glen_a=cfg.A*f,
                            fs=cfg.FS)
    ...
    if With_calving:
        ...
        for gdir in gdirs:
            if gdir.terminus_type == 'Marine-terminating':
                ...
                tasks.volume_inversion(gdir, glen_a=cfg.A*f, fs=cfg.FS)
